...
def main():
    parser = argparse.ArgumentParser(description='Include rows of data by product IDs')
    ...
    for country_dir in os.listdir(args.dir):
        if not os.path.isfile(os.path.join(args.dir, country_dir)):
            csv_file_path = os.path.join(args.dir, country_dir, country_dir + "_hs4_2010_to_2020.csv")
            print(csv_file_path)
            data = pd.read_csv(csv_file_path)
            data = data[data["product_id"].isin(hs4_product_ids)]
